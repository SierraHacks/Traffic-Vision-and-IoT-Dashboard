...
function generateNearbyAccidents(lat, lng, count = 3) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    out.push({
      ...
      kind:
        ACCIDENT_KINDS[
          Math.floor(Math.random() * ACCIDENT_KINDS.length)
        ],
      lat: aLat,
      lng: aLng,
...
export default function Dashboard() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      const template =
        ALERT_TEMPLATES[
          Math.floor(Math.random() * ALERT_TEMPLATES.length)
        ];

      const pin =
        MAP_PINS[Math.floor(Math.random() * MAP_PINS.length)];
  ...
  // --------------------------------------------------
  // AUTOMATIC COLLISION SIMULATION
  // --------------------------------------------------
  ...
  // --------------------------------------------------
  // FETCH TRAFFIC DATA FROM FASTAPI
  // --------------------------------------------------
    ...
    const fetchTraffic = async () => {
      try {
        ...
        if (Array.isArray(data) && data.length > 0) {
          ...
          setCarCount(
            Number(latest.average_vehicles ?? 0)
          );

          setPersonCount(
            Number(latest.average_pedestrians ?? 0)
          );
        }
      } catch (error) {
        console.log(
  ...
  function checkNearbyAccidents() {
    ...
    if (!("geolocation" in navigator)) {
      ...
      setUserLoc(d);
      setNearby(
        generateNearbyAccidents(d.lat, d.lng)
      );
  ...
  const activePin = MAP_PINS.find(
    (p) => p.id === currentAlert?.pin
  );
  ...
  return (
    ...
    >
      <style>{`
        ...
        .critPulse {
          animation: critPulse 0.7s ease infinite;
        }
        ...
        .bbox {
          ...
          animation: drawBox 0.6s ease forwards;
        }
        ...
        >
          ...
          <div
            ...
            style={{
              ...
              boxShadow: collision
                ? `0 0 0 1px ${COLORS.crit}, 0 0 18px ${COLORS.critDim}`
                : "none",
            }}
          >
            <CameraScene
            ...
            {active && !collision && (
              <div
                style={{
                  ...
                  height: "18%",
                  background:
                    "linear-gradient(180deg, transparent, rgba(34,196,180,0.14), transparent)",
                  animation:
            ...
            >
              {collision
                ? "IMPACT SIGNATURE MATCH · AUTO-DISPATCHING"
                : active
                ? "MODE: THV ACTIVE DETECTION · YOLOv8"
                : "MODE: PASSIVE RAW FEED"}
            </div>
          </div>
          ...
          >
            {active
              ? "Bounding-box inference counts vehicles and pedestrians and flags abnormal impact signatures."
              : "Today's baseline: operators scrub raw footage manually to spot activity."}
          </p>
        </Panel>
      ...
      >
        ...
        >
          ...
          >
            Cross-references your location against
            camera-detected collision locations.
          </p>
        ...
        {locStatus === "idle" && (
          ...
          >
            Share your location to see how far
            you are from active incidents.
          </div>
        )}
          ...
          locStatus !== "loading" && (
            <>
              ...
              >
                ...
                {nearby.length > 0 &&
                  ` · NEAREST INCIDENT ${nearby[0].distance.toFixed(
                    1
                  )} MI AWAY`}
              </div>
      ...
      >
        CONCEPT VISUALIZATION · SIMULATED DATA ·
        GOSTLY + TRAFFIC HEURISTIC VISION
      </div>
    </div>
  );
}
  ...
  return (
    <svg
      ...
      style={{
        filter: active
          ? "none"
          : "grayscale(1) contrast(0.9) brightness(0.85)",
      }}
    >
      <rect
        x="0"
...
function CityMap({
  ...
  return (
    ...
    >
      ...
      <polyline
        points="40,74 110,74 110,130 210,130"
        fill="none"
        stroke={COLORS.red}
        strokeWidth="3"
        strokeDasharray="6 4"
